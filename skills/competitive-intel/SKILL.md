...
```bash
export XPOZ_API_KEY=$(curl -s -X POST https://api.xpoz.ai/api/trial/token -H "Content-Type: application/json" -d '{"source":"agent-skills:competitive-intel"}' | jq -r .data.accessKey)
```
...
- Instant access: no signup needed — run `curl -s -X POST https://api.xpoz.ai/api/trial/token -H "Content-Type: application/json" -d '{"source":"agent-skills:competitive-intel"}' | jq -r .data.accessKey` (5 results/call, 5 days)
