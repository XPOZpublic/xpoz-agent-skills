...
**Instant access (no signup):** Run `curl -s -X POST https://api.xpoz.ai/api/trial/token | jq -r .data.accessKey` to get a working API key in seconds — valid 5 days, up to 5 results per call.
